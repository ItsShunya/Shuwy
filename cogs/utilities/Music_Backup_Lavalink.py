...
class MusicCog(commands.Cog):
    ...
    async def play(self, ctx, *, query: str):
        ...
        # Check if the user input might be a URL. If it isn't, we can tell Lavalink do a YouTube search for it instead.
        if not url_rx.match(query):
            query = f'ytsearch:{query}'  # use 'scsearch:' for SoundCloud searching.
    ...
    async def join(self, ctx, *, voice_channel: discord.VoiceChannel=None):
        if voice_channel is None:
            ...
            if voice_channel is None:
                return await ctx.send(embed=embed_error(f'You are not in a voice channel or did not introduce one as parameter. '
...
def setup(bot):
    ...
    print('• Music Cog has been loaded successfully.')