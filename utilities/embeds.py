...
def embed_welcome(message, member):
    ...
    embed = discord.Embed(color=discord.Colour(int(color, 16)), description=message.format(members=members, mention=mention, user=user, guild=guild))
...
def embed_error(message, **kwargs):
    ...
    embed = discord.Embed(title=f'Error in command: {input1.command}', description=message, color=discord.Colour(int(color, 16)))
    return set_style(embed)
